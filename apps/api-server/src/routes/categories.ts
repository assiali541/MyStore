...
router.post("/admin/categories", requireAdmin, async (req, res): Promise<void> => {
  ...
  const [cat] = await db.select().from(categoriesTable).where(eq(categoriesTable.id, insertId)).limit(1);
...
router.put("/admin/categories/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  // Pre-check existence
  const [existing] = await db.select().from(categoriesTable).where(eq(categoriesTable.id, params.data.id)).limit(1);
  if (!existing) {
    res.status(404).json({ error: "Category not found" });
    return;
  }
  ...
  const [cat] = await db.select().from(categoriesTable).where(eq(categoriesTable.id, params.data.id)).limit(1);
...
router.delete("/admin/categories/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  // Pre-fetch before delete to confirm existence
  const [toDelete] = await db.select().from(categoriesTable).where(eq(categoriesTable.id, params.data.id)).limit(1);
  if (!toDelete) {
    res.status(404).json({ error: "Category not found" });
    return;
  }
