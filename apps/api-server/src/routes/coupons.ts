...
router.post("/admin/coupons", requireAdmin, async (req, res): Promise<void> => {
  ...
  const [coupon] = await db.select().from(couponsTable).where(eq(couponsTable.id, insertId)).limit(1);
...
router.delete("/admin/coupons/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  const [toDelete] = await db.select().from(couponsTable).where(eq(couponsTable.id, params.data.id)).limit(1);
  if (!toDelete) {
