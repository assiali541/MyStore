...
router.post("/admin/products", requireAdmin, async (req, res): Promise<void> => {
  ...
  const [product] = await db.select().from(productsTable).where(eq(productsTable.id, insertId)).limit(1);
  const [enriched] = await enrichProducts([product!]);
...
router.delete("/admin/products/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  const [toDelete] = await db.select().from(productsTable).where(eq(productsTable.id, params.data.id)).limit(1);
  if (!toDelete) {
    res.status(404).json({ error: "Product not found" });
    return;
  }
